...
class ModelSelector:
    ...
    def select_model(self, query: str, requested_level: Optional[str] = None, conversation_context: Optional[Dict] = None) -> Tuple[str, str, str, Optional[float], Optional[str]]:
        """
        Select optimal model for query using ComplexityAnalyzer with conversation context
        ...
        if conversation_context:
            # Get the last used model from conversation history
            last_interaction = conversation_context.get('last_interaction')
            if last_interaction:
                previous_model = last_interaction.get('model_used')
                previous_level = last_interaction.get('intelligence_level', 'standard')

            # Check conversation length
            conversation_length = len(conversation_context.get('history', []))

            # CONTEXT CONTINUITY: For ongoing conversations, prefer consistency
            if previous_model and conversation_length > 1:
                # If this is a follow-up or clarification, maintain model level
                if any(phrase in query_lower for phrase in [
                    "continue", "also", "what about", "follow up", "furthermore",
                    "additionally", "can you also", "what if", "how about"
                ]):
                    return (previous_model, previous_level,
                           f"Conversation continuity - maintaining {previous_level} level",
                           None, None)

                # For short queries in ongoing conversations, don't downgrade
                if len(query) < 50 and previous_level in ["professional", "expert", "genius"]:
                    return (previous_model, previous_level,
                           f"Short query in ongoing {previous_level} conversation",
                           None, None)
        ...
        if self.use_analyzer:
            try:
                # Prepare context with conversation information
                context = {
