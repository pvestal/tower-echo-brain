...
class FeatureStore:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_values_entity ON feature_values(entity_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_values_timestamp ON feature_values(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_quality_feature ON feature_quality(feature_id)"
            )
    ...
    def _generate_id(self, prefix: str = "feat") -> str:
        ...
        random_suffix = hashlib.md5(
            str(np.random.random()).encode()).hexdigest()[:8]
    ...
    async def _store_values_sqlite(
        self, feature_id: str, values: List[FeatureValue]
    ) -> bool:
        ...
        self.logger.info(
            f"Stored {len(values)} values for feature {feature_id} in SQLite"
        )
        return True

    async def _store_values_parquet(
        self, feature_id: str, values: List[FeatureValue]
    ) -> bool:
        ...
        for date, group in df.groupby("date"):
            ...
            if parquet_path.exists():
                ...
                combined_df = pd.concat(
                    [existing_df, group.drop("date", axis=1)])
        ...
        self.logger.info(
            f"Stored {len(values)} values for feature {feature_id} in Parquet"
        )
        return True
    ...
    async def get_feature_values(
        ...
        if use_cache and self.redis_client:
            cache_key = self._get_cache_key(
                feature_id, entity_ids, start_time, end_time, version
            )
        ...
        # Try Parquet first
        parquet_values = await self._load_values_parquet(
            feature_id, entity_ids, start_time, end_time, version
        )
        if parquet_values:
            ...
            # Fall back to SQLite
            sqlite_values = await self._load_values_sqlite(
                feature_id, entity_ids, start_time, end_time, version
            )
        ...
        if use_cache and self.redis_client and values:
            # 1 hour TTL
            await self._store_in_cache(cache_key, values, ttl=3600)
    ...
    async def _load_values_parquet(
        ...
        if start_time or end_time:
            start_date = (
                start_time.date() if start_time else datetime(2020, 1, 1).date()
            )
        ...
        # Apply filters
        if entity_ids:
            combined_df = combined_df[combined_df["entity_id"].isin(
                entity_ids)]

        if start_time:
            combined_df = combined_df[
                pd.to_datetime(combined_df["timestamp"]) >= start_time
            ]

        if end_time:
            combined_df = combined_df[
                pd.to_datetime(combined_df["timestamp"]) <= end_time
            ]
        ...
        for _, row in combined_df.iterrows():
            feature_value = FeatureValue(
                ...
                version=row["version"],
                metadata=json.loads(
                    row["metadata"]) if row["metadata"] else None,
            )
            values.append(feature_value)
    ...
    async def get_feature_set_values(
        self, feature_set_id: str, entity_ids: List[str], timestamp: datetime = None
    ) -> pd.DataFrame:
        ...
        for feature_id in feature_set.features:
            feature_values = await self.get_feature_values(
                feature_id=feature_id, entity_ids=entity_ids, end_time=timestamp
            )
            ...
            for value in feature_values:
                if (
                    value.entity_id not in entity_values
                    or value.timestamp > entity_values[value.entity_id]["timestamp"]
                ):
            ...
            for entity_id in entity_ids:
                ...
                all_data[entity_id][feature_name] = entity_values.get(
                    entity_id, {}
                ).get("value")
    ...
    async def compute_feature(
        ...
        if not feature_def.computation_logic:
            raise ValueError(
                f"No computation logic defined for feature {feature_id}")
        ...
        try:
            ...
            computed_values = await self._execute_computation_logic(
                feature_def.computation_logic, entity_ids, start_time, end_time
            )
            ...
            self.logger.error(
                f"Feature computation failed for {feature_id}: {e}")
            raise
    ...
    async def monitor_feature_quality(self, feature_id: str) -> Dict[str, float]:
        ...
        values = await self.get_feature_values(
            feature_id=feature_id, start_time=start_time, end_time=end_time
        )
    ...
    async def _check_feature_validity(
        self, feature_id: str, values: List[FeatureValue]
    ) -> float:
        ...
        for value in values:
            ...
            # Check range
            if "min_value" in validation_rules and isinstance(
                value.value, (int, float)
            ):
                if value.value < validation_rules["min_value"]:
                    is_valid = False

            if "max_value" in validation_rules and isinstance(
                value.value, (int, float)
            ):
    ...
    async def _check_feature_consistency(
        self, feature_id: str, values: List[FeatureValue]
    ) -> float:
    ...
    async def _store_in_cache(
        self, cache_key: str, values: List[FeatureValue], ttl: int = 3600
    ) -> bool:
...
async def example_feature_store():
    ...
    df = await store.get_feature_set_values(
        feature_set_id=feature_set_id, entity_ids=[
...
if __name__ == "__main__":
    ...
    async def store_values_endpoint(feature_id: str, values: List[Dict[str, Any]]):
        try:
            ...
            success = await feature_store.store_feature_values(
    ...
    async def create_feature_set_endpoint(
        name: str, description: str, features: List[str], entity_type: str
    ):
        try:
    ...
    async def get_feature_set_values_endpoint(
        feature_set_id: str, entity_ids: List[str]
    ):
        try:
