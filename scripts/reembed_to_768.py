#!/usr/bin/env python3
"""
Re-embed all 52,300 records from embedding_cache using nomic-embed-text (768-dim)
Stores results in Qdrant collection "echo_memory_768"
...
log_file = log_dir / 'reembedding.log'
...
class ReembeddingProcessor:
    def __init__(self):
        ...
        self.collection_name = "echo_memory_768"
    ...
    async def generate_embedding(self, text: str) -> Optional[List[float]]:
        ...
        if len(text) > MAX_TEXT_LENGTH:
            ...
            logger.warning(f"Truncated text to {len(text)} chars")
...
""")
    ...
    async def run_test_batch(self, conn: asyncpg.Connection) -> float:
        ...
        logger.info("Running test batch of 10 records...")
...
""")
    ...
    async def run(self, test_mode: bool = False):
        ...
        logger.info("Starting reembedding processor...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Re-embed vectors to 768 dimensions')
    ...
    if args.status:
        ...
        await processor.run(test_mode=args.test)
