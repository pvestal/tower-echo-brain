...
from echo_deployment_manager import DeploymentManager, DeploymentConfig, EnvironmentType
...
class GitManager:
    ...
    async def clone_or_update_repo(self, repo_url: str, branch: str, commit: str) -> Tuple[bool, str]:
    ...
    async def get_changed_files(self, repo_path: str, base_commit: str, target_commit: str) -> List[str]:
        """Get list of changed files between commits"""
        try:
            repo = git.Repo(repo_path)
            diff = repo.git.diff('--name-only', f"{base_commit}..{target_commit}")
            return diff.split('\n') if diff else []
        except Exception:
            return []
...
class SecurityScanner:
    ...
    async def scan_code(self, repo_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results['overall_score'] = self._calculate_security_score(results['scan_results'])
...
class PipelineExecutor:
    ...
    async def execute_pipeline(self, pipeline_run: PipelineRun) -> PipelineRun:
        ...
        try:
            ...
            await self._execute_stage(pipeline_run, PipelineStage.CODE_ANALYSIS, self._code_analysis_stage)
            
            # Stage 2: Test Generation
            if pipeline_run.config.run_tests:
                await self._execute_stage(pipeline_run, PipelineStage.TEST_GENERATION, self._test_generation_stage)
                await self._execute_stage(pipeline_run, PipelineStage.TEST_EXECUTION, self._test_execution_stage)
            
            # Stage 3: Security Scan
            await self._execute_stage(pipeline_run, PipelineStage.SECURITY_SCAN, self._security_scan_stage)
            
            # Stage 4: Build
            await self._execute_stage(pipeline_run, PipelineStage.BUILD, self._build_stage)
            
            # Stage 5: Deployment
            if pipeline_run.config.deploy_on_success:
                await self._execute_stage(pipeline_run, PipelineStage.DEPLOYMENT, self._deployment_stage)
                await self._execute_stage(pipeline_run, PipelineStage.HEALTH_CHECK, self._health_check_stage)
            
            # Stage 6: Notification
            if pipeline_run.config.notify_on_completion:
                await self._execute_stage(pipeline_run, PipelineStage.NOTIFICATION, self._notification_stage)
            
            pipeline_run.status = PipelineStatus.SUCCESS
            pipeline_run.end_time = datetime.now()
            pipeline_run.logs.append(f"Pipeline {pipeline_run.id} completed successfully")
    ...
    async def _execute_stage(self, pipeline_run: PipelineRun, stage: PipelineStage, stage_func):
        ...
        try:
            ...
            pipeline_run.logs.append(f"Stage {stage.value} completed successfully")
    ...
    async def _code_analysis_stage(self, pipeline_run: PipelineRun) -> Dict[str, Any]:
        ...
        # Get commit information
        commit_info = await self.git_manager.get_commit_info(repo_path, config.git_commit)
    ...
    async def _test_generation_stage(self, pipeline_run: PipelineRun) -> Dict[str, Any]:
        ...
        changed_files = pipeline_run.stages[PipelineStage.CODE_ANALYSIS]['result']['changed_files']
        ...
        return {
            'generated_tests': len(test_results),
            'test_results': test_results
        }
    
    async def _test_execution_stage(self, pipeline_run: PipelineRun) -> Dict[str, Any]:
        """Execute generated tests"""
        test_results = pipeline_run.stages[PipelineStage.TEST_GENERATION]['result']['test_results']
        ...
        for test_result in test_results:
            ...
            if result.get('success'):
                total_tests += result.get('test_suite', {}).get('test_count', 0)
                if result.get('results', {}).get('success'):
                    passed_tests += result.get('test_suite', {}).get('test_count', 0)
                else:
                    failed_tests += result.get('test_suite', {}).get('test_count', 0)
                
                coverage = result.get('results', {}).get('coverage', {}).get('percentage', 0)
                coverage_sum += coverage
        
        avg_coverage = coverage_sum / len(test_results) if test_results else 0
        
        # Fail pipeline if test failure rate is too high
        if total_tests > 0 and (failed_tests / total_tests) > 0.1:  # 10% failure threshold
            raise Exception(f"Test failure rate too high: {failed_tests}/{total_tests} failed")
    ...
    async def _security_scan_stage(self, pipeline_run: PipelineRun) -> Dict[str, Any]:
        ...
        if security_results.get('overall_score', 0) < 70:  # 70% minimum security score
            raise Exception(f"Security score too low: {security_results['overall_score']}/100")
        ...
        for env in config.target_environments:
            try:
                deployment_config = DeploymentConfig(
                    ...
                    port=8309 if env == 'production' else 8310,  # Different ports for different envs
        ...
        for deployment in pipeline_run.deployment_results:
            if deployment['status'] == 'initiated':
                ...
                health_results.append({
                    ...
                    'health_status': deployment_status.get('health_status', False) if deployment_status else False,
                    'deployment_status': deployment_status.get('status', 'unknown') if deployment_status else 'unknown'
                })
        
        # Check if any critical environments failed
        for result in health_results:
            if result['environment'] == 'production' and not result['health_status']:
                raise Exception(f"Production deployment health check failed: {result['deployment_id']}")
    ...
    async def _notification_stage(self, pipeline_run: PipelineRun) -> Dict[str, Any]:
        ...
        try:
            ...
            notification_data = {
                ...
                'duration': (pipeline_run.end_time - pipeline_run.start_time).total_seconds() if pipeline_run.end_time else None,
                'test_results': pipeline_run.test_results,
...
class EchoCICDPipeline:
    ...
    async def get_pipeline_status(self, pipeline_id: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if row:
                ...
                # Parse JSON fields
                for field in ['config', 'stages', 'artifacts', 'logs', 'test_results', 'deployment_results']:
    ...
    async def list_pipelines(self, app_name: str = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def _save_pipeline(self, pipeline_run: PipelineRun):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            ''', (
                ...
                json.dumps({k.value: v for k, v in pipeline_run.stages.items()}, default=str),
    ...
    async def _update_echo_learning(self, pipeline_run: PipelineRun):
        ...
        try:
            learning_data = {
                ...
                'duration': (pipeline_run.end_time - pipeline_run.start_time).total_seconds() if pipeline_run.end_time else None,
                'stages_completed': len([s for s in pipeline_run.stages.values() if s.get('status') == 'success']),
                'test_coverage': pipeline_run.test_results.get('average_coverage', 0) if pipeline_run.test_results else 0,
                'security_score': pipeline_run.stages.get(PipelineStage.SECURITY_SCAN, {}).get('result', {}).get('overall_score', 0),
                'app_name': pipeline_run.config.app_name,
                'git_commit': pipeline_run.config.git_commit
            }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        print(f"Failed to update Echo learning: HTTP {resp.status}")
