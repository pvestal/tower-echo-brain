...
async function fetchMetrics() {
  try {
    ...
    const metricsRes = await fetch('http://***REMOVED***:8309/api/echo/improvement/metrics')
    if (metricsRes.ok) {
      metrics.value = await metricsRes.json()
    }

    // Fetch status
    const statusRes = await fetch('http://***REMOVED***:8309/api/echo/improvement/status')
    if (statusRes.ok) {
      status.value = await statusRes.json()
    }

    // Fetch knowledge graph
    const graphRes = await fetch('http://***REMOVED***:8309/api/echo/improvement/knowledge-graph')
    if (graphRes.ok) {
    ...
    const dbRes = await fetch('http://***REMOVED***:8309/api/echo/db/stats')
    if (dbRes.ok) {
...
async function triggerImprovement() {
  try {
    const res = await fetch('http://***REMOVED***:8309/api/echo/improvement/trigger', {
      method: 'POST'
    })
    if (res.ok) {
      await fetchMetrics()
