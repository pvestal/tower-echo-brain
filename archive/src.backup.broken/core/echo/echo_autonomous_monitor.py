...
class EchoAutonomousMonitor:
    def __init__(self):
        self.services = {
            ...
            "kb": {"port": 8307, "health_endpoint": "/api/kb/health", "critical": True},
            "dashboard": {"port": 8080, "critical": False},
            "comfyui": {"port": 8188, "critical": False},
        }
    ...
    async def check_service_health(self, name: str, config: dict) -> bool:
        ...
        if "systemd_service" in config:
            ...
            if result.stdout.strip() != "active":
                logger.warning(
                    f"Service {name} systemd status: {result.stdout.strip()}"
                )
                return False
                ...
                try:
                    ...
                    if log_path.exists():
                        ...
                        if (
                            ...
                            logger.warning(
                                f"⚠️ Errors detected in {name} logs")
    ...
    async def attempt_repairs(self):
        ...
        logger.info(
            f"🔧 Attempting to repair {len(self.issues_detected)} issues...")
                ...
                if "systemd_service" in service_config:
                    ...
                    result = subprocess.run(
                        [
                            ...
                            "systemctl",
                            "restart",
                            service_config["systemd_service"],
                        ],
                        capture_output=True,
                        text=True,
                    )
                    ...
                    if result.returncode == 0:
                        logger.info(
                            f"✅ Successfully restarted {issue['service']}")
                        ...
                        logger.error(
                            f"❌ Failed to restart {issue['service']}: {result.stderr}"
                        )

            except Exception as e:
                logger.error(
                    f"Repair attempt failed for {issue['service']}: {e}")

        # Report status
        logger.info(
            f"📊 Repairs attempted: {len(self.repairs_attempted)}, Issues remaining: {len(self.issues_detected)}"
        )
