...
class ProactiveImprovementEngine:
    ...
    def __init__(self):
        self.improvement_log = Path(
            "/opt/tower-echo-brain/logs/proactive_improvements.log"
        )
    ...
    async def search_for_improvements(
            self, project_path: str) -> List[Dict[str, Any]]:
    ...
    async def auto_fix_file(self, file_path: str) -> Dict[str, Any]:
        ...
        results = {"file": file_path, "fixes_applied": [], "success": False}
        ...
        try:
            ...
            autopep8_result = subprocess.run(
                [
                    "/opt/tower-echo-brain/venv/bin/autopep8",
                    "--in-place",
                    "--aggressive",
                    file_path,
                ],
            ...
            ruff_result = subprocess.run(
                ["/opt/tower-echo-brain/venv/bin/ruff",
                    "check", "--fix", file_path],
                capture_output=True,
                timeout=30,
            )
    ...
    async def commit_improvements(
        self, project_path: str, files: List[str], message: str
    ) -> bool:
        """Commit improvements using git (requires permission)"""
        if not self.auto_commit_enabled:
            logger.info(
                f"Would commit {
                    len(files)} files with message: {message}"
            )
            return False
            ...
            try:
                for project_path in project_paths:
                    ...
                    if improvements:
                        logger.info(
                            f"📋 Found {
                                len(improvements)} potential improvements"
                        )

                        # Group by file
                        files_to_fix = {}
                        # Limit to top 10 per cycle
                        ...
                        if fixed_files and self.auto_commit_enabled:
                            await self.commit_improvements(
                                ...
                                fixed_files,
                                f"Auto-fixed {
                                    len(fixed_files)} files with code improvements",
                            )
