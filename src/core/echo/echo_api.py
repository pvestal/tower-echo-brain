...
from src.core.echo.echo_temporal_db_integration import (TemporalDatabaseManager,
                                          setup_temporal_database)
from fastapi import BackgroundTasks, FastAPI, HTTPException
from fastapi.responses import JSONResponse, StreamingResponse
from pydantic import BaseModel

from src.core.echo.echo_self_awareness import EchoCapabilityEndpoint, EchoSelfAwareness
# Import new modules
from src.misc.temporal_reasoning import (EchoTemporalInterface, TemporalEvent,
                                TemporalEventType)
...
async def temporal_query(query: TemporalQuery):
    ...
    if (
        db_manager
        and query.type == "verify_causality"
        and result.get("causal_chain_exists")
    ):
        chain = {
...
async def validate_temporal_consistency(events: List[Dict]):
    ...
    if db_manager:
        timeline_id = events[0].get(
            "timeline_id", "main") if events else "main"
...
async def process_message(message: ChatMessage):
    ...
    if (
        "capabilities" in message.message.lower()
        or "what can you do" in message.message.lower()
    ):
        test = CapabilityTest(
            test_type="self_identification", query=message.message)
...
async def get_stats():
    ...
    if db_manager:
        ...
        async with db_manager.pool.acquire() as conn:
            stats["temporal_events"] = await conn.fetchval(
                "SELECT COUNT(*) FROM temporal_events"
            )
            stats["paradoxes_detected"] = await conn.fetchval(
                "SELECT COUNT(*) FROM temporal_paradoxes"
            )
            stats["capabilities_tracked"] = await conn.fetchval(
                "SELECT COUNT(*) FROM capability_evolution"
            )
            stats["self_reports_generated"] = await conn.fetchval(
                "SELECT COUNT(*) FROM self_awareness_reports"
            )
    ...
    return {
        ...
        "categories": {
            ...
            "self_awareness": [
                ...
                "/api/echo/self-analysis",
                "/api/echo/capabilities",
            ],
            "temporal": [
                "/api/echo/temporal/query",
                "/api/echo/temporal/validate",
                "/api/echo/temporal/paradoxes",
            ],
