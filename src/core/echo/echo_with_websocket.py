...
# Store connected cognitive clients
cognitive_clients: List[WebSocket] = []

@app.websocket("/ws/cognitive")
async def cognitive_websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Send cognitive processing response
            await websocket.send_text(f"Echo Brain: Processing {data}")
            
            # Broadcast to all cognitive visualization clients
            for client in cognitive_clients:
                if client != websocket:
                    await client.send_text(f"Cognitive update: {data}")
