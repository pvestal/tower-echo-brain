...
class EchoBrainThoughts:
    ...
    async def start_thinking(self, thought_type: str, query: str) -> str:
        ...
        logger.info(
            f"🧠 ECHO THINKING: {thought_type} | Query: {query[:50]}...")

        return thought_id

    async def fire_neurons(
        self,
        thought_id: str,
        ...
        thought = next(
            (t for t in self.thought_history if t["id"] == thought_id), None)
        if not thought:
            return
        ...
        logger.info(
            f"🔥 NEURONS FIRING: {neuron_display} | {intensity_bar} | {thought_content}"
        )
    ...
    async def analyze_complexity(self, thought_id: str, complexity_score: float):
        ...
        await self.fire_neurons(
            thought_id, [NeuronType.ANALYSIS,
                         NeuronType.LOGICAL], intensity, analysis
        )
    ...
    async def finish_thinking(self, thought_id: str):
        ...
        thought["duration"] = (
            thought["end_time"] - thought["start_time"]
        ).total_seconds()
        ...
        logger.info(
            f"🧠 THOUGHT COMPLETE: Duration {thought['duration']:.2f}s | Neurons: {len(thought['neurons_activated'])}"
        )
        ...
        return {
            ...
            "active_regions": {
                region: {
                    ...
                    "neurons_active": len(
                        [n for n in data["neurons"] if n in self.active_neurons]
                    ),
                }
                for region, data in self.neuron_regions.items()
            },
