#!/usr/bin/env python3
"""
Unified Conversation System for Echo Brain
Integrates all conversation, memory, and context systems into one cohesive handler
"""
...
class UnifiedConversationSystem:
    ...
    def __init__(self):
        ...
        # Unified cache for all conversation data
        self.unified_cache = {}
        ...
        logger.info("🔄 Unified Conversation System initialized")
...
# Global instance
unified_conversation = UnifiedConversationSystem()


# Helper functions for easy integration
async def process_unified_message(conversation_id: str, user_query: str,
                                 user_id: str = None, metadata: Dict = None) -> Dict:
    """Process a message through the unified system"""
    return await unified_conversation.process_message(
        conversation_id, user_query, user_id, metadata
    )


async def save_unified_response(conversation_id: str, response: str, metadata: Dict = None):
    """Save response through the unified system"""
    await unified_conversation.save_response(conversation_id, response, metadata)