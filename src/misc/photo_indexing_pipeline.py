...
class PhotoIndexer:
    def __init__(self):
        self.photos_path = Path(
            "/mnt/10TB2/Google_Takeout_2025/Takeout/Google Photos")
        ...
        for root, dirs, files in os.walk(self.photos_path):
            for file in files:
                if file.lower().endswith(
                    ...
                    cursor.execute(
                        "SELECT id FROM photos WHERE file_path = ?", (str(
                            filepath),)
                    )
                    if cursor.fetchone():
                        skipped += 1
                    ...
                    try:
                        ...
                        perceptual_hash = self.calculate_perceptual_hash(
                            filepath)

                        # Check for duplicates
                        cursor.execute(
                            "SELECT file_path FROM photos WHERE file_hash = ?",
                            (file_hash,),
                        )
                        ...
                        if duplicate:
                            duplicates += 1
                            print(
                                f"  Duplicate found: {filepath.name} == {Path(duplicate[0]).name}"
                            )
                            continue
                        ...
                        if indexed % 100 == 0:
                            conn.commit()
                            print(
                                f"  Indexed: {indexed}, Skipped: {skipped}, Duplicates: {duplicates}"
                            )
...
""",
            ...
            response = requests.post(
                "https://localhost/kb/api/articles", json=kb_article, verify=False
            )
