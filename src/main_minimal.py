...
try:
    ...
    logger.warning(f"⚠️ Some enhanced modules not available: {e}")
...
try:
    ...
    logger.info("✅ Code reviewer connected")
except ImportError as e:
    logger.warning(f"Code reviewer not available: {e}")

try:
    from src.tasks.code_refactor_executor import CodeRefactorExecutor

    code_refactor_executor = CodeRefactorExecutor()
    logger.info("✅ Code refactor executor connected")
except ImportError as e:
    logger.warning(f"Code refactor executor not available: {e}")

try:
    from src.tasks.anime_code_reviewer import AnimeCodeReviewer

    anime_reviewer = AnimeCodeReviewer()
    logger.info("✅ Anime code reviewer connected")
except ImportError as e:
    logger.warning(f"Anime reviewer not available: {e}")

logger.info("✅ CODE CAPABILITIES LOADED - Echo can now review and refactor code!")

# Import existing modules that remain external

# Logging already configured earlier
...
# app.include_router(director_router)  # TEMPORARILY DISABLED - missing file
logger.info("🎬 Director workflow API routes loaded")
# Include soundtrack BPM analysis routes
# app.include_router(soundtrack_router)  # TEMPORARILY DISABLED - missing file
...
async def startup_event():
    ...
    # Initialize board API - TEMPORARILY DISABLED
    # try:
    ...
    logger.info("⚠️ Board of Directors API temporarily disabled - missing board_api.py")
...
if __name__ == "__main__":
    uvicorn.run(
        "src.main:app", host="127.0.0.1", port=8309, reload=False, access_log=True
    )
