...
class DecisionBranchingEngine:
    ...
    async def create_branch_from_decision(self, decision_id: int, option_id: int) -> int:
        ...
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                cur.execute("""
                    SELECT td.*, do.option_text, do.target_branch_id, tb.branch_name as parent_branch_name
                    FROM timeline_decisions td
                    JOIN decision_options do ON "do".decision_id = td.id
    ...
    async def apply_decision_consequences(self, option_id: int, branch_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                cur.execute("""
                    SELECT dc.*, do.immediate_consequences, do.delayed_consequences
                    FROM decision_consequences dc
                    JOIN decision_options do ON "do".id = dc.option_id
    ...
    async def _check_plot_logic(self, cur, timeline_id: int) -> List[Dict[str, Any]]:
        ...
        cur.execute("""
            ...
            FROM timeline_decisions td
            JOIN timeline_branches tb ON tb.id = td.branch_id
            LEFT JOIN decision_options do ON "do".decision_id = td.id
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute("""
                ...
                ORDER BY do.probability_weight DESC
            """, (decision_id,))
