            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"{self.base_url}:{port}/api/health", timeout=10
                    ) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            print(
                                f"  ✅ {service_name} (:{port}): {data.get('status', 'unknown')}"
                            )
                        else:
                            print(
                                f"  ❌ {service_name} (:{port}): HTTP {resp.status}")
                            all_healthy = False
            except Exception as e:
...
'''
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            test_count = data.get(
                                "test_suite", {}).get("test_count", 0)
                            print(
                                f"  ✅ Generated {test_count} tests successfully")
                            return success
                        else:
                            print(
                                f"  ❌ Test generation failed: HTTP {resp.status}")
                            return False
            finally:
                        ...
                        if resp.status == 200:
                            ...
                            async with session.get(
                                ...
                                if status_resp.status == 200:
                                    ...
                                    print(
                                        f"  ✅ Deployment status: {status_data.get('status', 'unknown')}"
                                    )
                                    return True
                                else:
                                    print(
                                        f"  ❌ Failed to get deployment status: HTTP {status_resp.status}"
                                    )
                                    return False
                        else:
                    ...
                    if resp.status == 200:
                        ...
                        async with session.get(
                            ...
                            if status_resp.status == 200:
                                status_data = await status_resp.json()
                                print(
                                    f"  ✅ Pipeline status: {status_data.get('status', 'unknown')}"
                                )
                                return True
                            else:
                                print(
                                    f"  ❌ Failed to get pipeline status: HTTP {status_resp.status}"
                                )
                                return False
                    else:
                        print(
                            f"  ❌ Pipeline trigger failed: HTTP {resp.status}")
                        return False
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if "pipeline_id" in data:
                            print(
                                f"  ✅ Pipeline triggered: {data['pipeline_id']}")

                        return status in ["processed", "ignored"]
                    else:
                        print(
                            f"  ❌ Webhook processing failed: HTTP {resp.status}")
                        return False
                    ...
                    if resp.status == 200:
                        ...
                        async with session.get(
                            ...
                            if insights_resp.status == 200:
                                ...
                                print(
                                    f"  ✅ Learning insights retrieved: {insights.get('learning_status')}"
                                )
                                return True
                            else:
                                print(
                                    f"  ❌ Failed to get insights: HTTP {insights_resp.status}"
                                )
                                return False
                    else:
                        print(
                            f"  ❌ Feedback recording failed: HTTP {resp.status}")
                        return False
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if resp.status != 200:
                        print(
                            f"  ❌ Failed to trigger pipeline: HTTP {resp.status}")
                        return False
                ...
                for i in range(6):  # Check for 30 seconds
                    ...
                    async with session.get(
                        ...
                        if status_resp.status == 200:
                            ...
                            print(
                                f"  🔄 Pipeline status: {status} (stage: {current_stage or 'none'})"
                            )
                            ...
                            if status in ["success", "failed", "cancelled"]:
                                ...
                                async with session.post(
                                    ...
                                    if feedback_resp.status == 200:
                                        print(
                                            f"  ✅ Learning feedback sent successfully"
                                        )
                                    else:
                                        print(
                                            f"  ⚠️ Learning feedback failed: HTTP {feedback_resp.status}"
                                        )
                ...
                print(
                    f"  ⏰ Pipeline still running after 30 seconds - considering success"
                )
                return True
    ...
    async def generate_test_report(self):
        ...
        report_file = (
            f"/opt/tower-echo-brain/test-reports/cicd-test-{int(time.time())}.json"
        )
