...
class EchoEvolutionTester:
    ...
    async def test_service_health(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/echo/health", timeout=10)
            if response.status_code == 200:
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Service Health Check", False, f"Missing field: {field}"
                        )
                        return False
                ...
                if data["status"] != "healthy":
                    self.log_test_result(
                        ...
                        False,
                        f"Unhealthy status: {data['status']}",
                    )
                    return False
                ...
                self.log_test_result(
                    "Service Health Check", False, f"HTTP {response.status_code}"
                )
                return False
    ...
    async def test_evolution_status(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/echo/evolution/status", timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                required_fields = [
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Evolution Status", False, f"Missing field: {field}"
                        )
                        return False
                ...
                self.log_test_result(
                    "Evolution Status", False, f"HTTP {response.status_code}"
                )
                return False
    ...
    async def test_git_status(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/echo/evolution/git-status", timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                required_fields = [
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Git Status", False, f"Missing field: {field}"
                        )
                        return False
                ...
                if "current_branch" not in git_status:
                    self.log_test_result(
                        "Git Status", False, "Missing current_branch in git_status"
                    )
                    return False
                ...
                for capability in expected_capabilities:
                    if not capabilities.get(capability):
                        self.log_test_result(
                            "Git Status", False, f"Missing git capability: {capability}"
                        )
                        return False
                ...
                self.log_test_result(
                    "Git Status", False, f"HTTP {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Self-Analysis Trigger", False, f"Missing field: {field}"
                        )
                        return False
                ...
                if len(data["insights"]) == 0:
                    self.log_test_result(
                        "Self-Analysis Trigger", False, "No insights generated"
                    )
                    return False

                if len(data["action_items"]) == 0:
                    self.log_test_result(
                        "Self-Analysis Trigger", False, "No action items generated"
                    )
                    return False

                if len(data["capabilities"]) == 0:
                    self.log_test_result(
                        "Self-Analysis Trigger", False, "No capabilities analyzed"
                    )
                    return False
                ...
                self.log_test_result(
                    "Self-Analysis Trigger", False, f"HTTP {response.status_code}"
                )
                return False
    ...
    async def test_learning_metrics(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/echo/evolution/learning-metrics", timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                required_fields = [
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Learning Metrics", False, f"Missing field: {field}"
                        )
                        return False
                ...
                expected_state_fields = [
                    "conversations_since_last_analysis",
                ...
                for field in expected_state_fields:
                    if field not in current_state:
                        self.log_test_result(
                            "Learning Metrics", False, f"Missing state field: {field}"
                        )
                        return False
                ...
                self.log_test_result(
                    "Learning Metrics", False, f"HTTP {response.status_code}"
                )
                return False
    ...
    async def test_conversation_processing(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/echo/query", json=payload, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Conversation Processing", False, f"Missing field: {field}"
                        )
                        return False
                ...
                if len(data["response"]) < 50:
                    self.log_test_result(
                        "Conversation Processing", False, "Response too short"
                    )
                    return False
                ...
                if data["processing_time"] > 60:
                    self.log_test_result(
                        "Conversation Processing", False, "Processing time too long"
                    )
                    return False
                ...
                self.log_test_result(
                    "Conversation Processing", False, f"HTTP {response.status_code}"
                )
                return False
    ...
    async def test_manual_evolution_trigger(self) -> bool:
        ...
        try:
            ...
            if status_response.status_code != 200:
                self.log_test_result(
                    "Manual Evolution Trigger", False, "Cannot get initial status"
                )
                return False
            ...
            payload = {"reason": "comprehensive_testing_manual_trigger"}

            response = requests.post(
                f"{self.base_url}/api/echo/evolution/trigger", json=payload, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                for field in required_fields:
                    if field not in data:
                        self.log_test_result(
                            "Manual Evolution Trigger", False, f"Missing field: {field}"
                        )
                        return False

                if not data["success"]:
                    self.log_test_result(
                        "Manual Evolution Trigger", False, "Trigger reported failure"
                    )
                    return False
                ...
                self.log_test_result(
                    "Manual Evolution Trigger", False, f"HTTP {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if "current_branch" not in git_status:
                self.log_test_result(
                    ...
                    False,
                    "Git status missing current_branch",
                )
                return False
    ...
    async def test_self_analysis_components(self) -> bool:
        ...
        try:
            ...
            from src.core.echo.echo_self_analysis import AnalysisDepth, EchoSelfAnalysis
            ...
            if not self_analysis.capability_registry:
                self.log_test_result(
                    "Self-Analysis Components", False, "Empty capability registry"
                )
                return False
            ...
            if len(depth_levels) < 4:
                self.log_test_result(
                    "Self-Analysis Components",
                    False,
                    "Insufficient analysis depth levels",
                )
                return False
    ...
    async def test_evolution_orchestrator(self) -> bool:
        ...
        try:
            ...
            from src.core.echo.echo_autonomous_evolution import (EchoAutonomousEvolution,
                                                   EvolutionPhase,
                                                   EvolutionTrigger)
            ...
            for field in required_state_fields:
                if field not in evolution.learning_state:
                    self.log_test_result(
                        "Evolution Orchestrator", False, f"Missing state field: {field}"
                    )
                    return False
            ...
            if len(trigger_types) < 5:
                self.log_test_result(
                    "Evolution Orchestrator", False, "Insufficient trigger types"
                )
                return False
            ...
            if len(phase_types) < 6:
                self.log_test_result(
                    "Evolution Orchestrator", False, "Insufficient evolution phases"
                )
                return False
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                self.log_test_result(
                    test_name, False, f"Test execution error: {e}")
        ...
        logger.info(
            f"Success Rate: {(self.test_results['tests_passed'] / max(1, self.test_results['tests_run']) * 100):.1f}%"
        )
