...
class ServiceMonitor:
    ...
    async def check_service_port(self, port: int, endpoint: Optional[str] = None) -> bool:
    ...
    async def diagnose_service(self, service_id: str) -> Dict:
        ...
        if service["check_method"] == "systemd":
            ...
            if "health_check" in service:
                ...
                if not health.get("healthy"):
                    ...
                    if health.get("error_count", 0) > 50:
                        diagnosis["issues"].append(f"High error count: {health['error_count']}")
                    if health.get("process_count", 0) > 1:
                        diagnosis["issues"].append(f"Multiple processes: {health['process_count']}")
            ...
            if not port_open:
                diagnosis["issues"].append(f"Port {service['port']} not responding")
            ...
            try:
                ...
                for service_id in self.services:
                    ...
                    if not diagnosis.get("healthy"):
                        logger.warning(f"❌ {diagnosis['service']} unhealthy: {diagnosis.get('issues')}")
                        
                        # Attempt repair
                        logger.info(f"🔧 Attempting to repair {diagnosis['service']}")
                        repair_result = await self.repair_service(service_id)
                        
                        if repair_result["success"]:
                            logger.info(f"✅ {diagnosis['service']} repaired successfully")
                        else:
                            logger.error(f"❌ Failed to repair {diagnosis['service']}")
                    else:
                        logger.debug(f"✅ {diagnosis['service']} healthy")
                
                # Save status to file for Echo to read
                status_file = Path("/opt/tower-echo-brain/data/service_status.json")
