...
class EchoSelfAwareness:
    ...
    def _discover_endpoints(self) -> List[str]:
        ...
        if os.path.exists(echo_file):
            with open(echo_file, 'r') as f:
                ...
                import re
                patterns = re.findall(r'@app\.(get|post|put|delete|patch)\(["\']([^"\']*)["\']]', content)
    ...
    def _discover_models(self) -> List[str]:
        ...
        try:
            ...
            models = ['tinyllama:latest', 'deepseek-coder:latest', 'mistral:7b']
    ...
    async def generate_self_report(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        report = {
            ...
            'temporal_logic': {
                ...
                ] if self.temporal_capable else ['Not available']
            },
            'endpoints': {
...
"""
            ...
            if self.self_awareness.temporal_capable:
                explanation = await self.self_awareness.explain_capability('temporal_logic')
                return {
