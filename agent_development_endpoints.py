...
agent_dev_router = APIRouter(prefix="/api/agent-development", tags=["agent-development"])
...
async def discover_available_tools():
    ...
    try:
        ...
        return {
            ...
            "tool_categories": {
                "intelligence_systems": len([t for t in agent_developer.available_tools if t.get('type') == 'intelligence_system']),
                "tower_services": len([t for t in agent_developer.available_tools if t.get('type') == 'tower_service']),
                "system_tools": len([t for t in agent_developer.available_tools if t.get('type') == 'system_tool'])
            },
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
        logger.error(f"Error discovering tools: {e}")
        raise HTTPException(status_code=500, detail=f"Error discovering tools: {str(e)}")
    ...
    try:
        ...
        return {
            ...
            "capability_analysis": {
                ...
                "performance_metrics": {
                    test_name: {
                        "expected_vs_actual": cap["expected_level"] == cap["actual_level"],
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing capabilities: {str(e)}")
...
async def develop_new_agent(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error starting development: {str(e)}")

async def develop_agent_background(agent_spec: Dict, test_immediately: bool, deploy_immediately: bool):
    ...
    try:
        ...
        logger.info(f"🤖 Agent development completed: {agent_spec['name']} - Status: {development_session['status']}")
...
async def get_development_sessions():
    ...
    try:
        return {
            ...
            "session_summary": {
                "in_progress": len([s for s in agent_developer.development_sessions if s['status'] == 'in_progress']),
                "completed": len([s for s in agent_developer.development_sessions if s['status'] == 'completed']),
                "failed": len([s for s in agent_developer.development_sessions if s['status'] == 'failed'])
            },
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
...
async def get_development_session(agent_name: str):
    ...
    try:
        ...
        session = next(
            (s for s in agent_developer.development_sessions if s.get('agent_name') == agent_name),
            None
        )

        if not session:
            raise HTTPException(status_code=404, detail=f"Development session for {agent_name} not found")
...
async def get_agent_templates():
    ...
    templates = {
        "task_agent": {
            ...
            "example_requirements": ["execute complex tasks", "coordinate multiple tools", "handle failures gracefully"],
        ...
        "research_agent": {
            ...
            "example_requirements": ["search for information", "analyze findings", "generate reports"],
        ...
        "coordination_agent": {
            ...
            "default_capabilities": ["agent_management", "workflow_orchestration", "conflict_resolution"],
            "example_requirements": ["manage multiple agents", "orchestrate workflows", "resolve conflicts"],
        ...
        "analysis_agent": {
            ...
            "default_capabilities": ["data_processing", "pattern_recognition", "insight_generation"],
            "example_requirements": ["analyze data patterns", "generate insights", "create visualizations"],
            "recommended_tools": ["data_processing", "echo_brain", "visualization_tools"]
        },
        "communication_agent": {
            "name": "Communication Agent",
            "description": "Specialized in communication and interaction management",
            "default_capabilities": ["message_processing", "response_generation", "context_management"],
            "example_requirements": ["handle user interactions", "generate responses", "maintain context"],
